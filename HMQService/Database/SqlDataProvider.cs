...
namespace HMQService.Database
{
    internal class SqlDataProvider : IDataProvider
    {
        ...
        private SqlConnection GetSqlServerConnection()
        {
            ...
            {
                Log.GetLogger().ErrorFormat("GetSqlServerConnection catch an error : {0}", e.Message);
                return null;
            }
        }
        ...
        public int ExecuteNonQuery(string sql)
        {
            using (sqlConnection = this.GetSqlServerConnection())
            {
                ...
                {
                    Log.GetLogger().ErrorFormat("ExecuteNonQuery catch an error : {0}", e.Message);
        ...
        public DataSet RetriveDataSet(string sql)
        {
            ...
            using (sqlConnection = this.GetSqlServerConnection())
            {
                ...
                using (SqlDataAdapter da = new SqlDataAdapter(sql, sqlConnection))
                {
                    ...
                    {
                        Log.GetLogger().ErrorFormat("RetriveDataSet catch an error : {0}", e.Message);
                    }
                    return ds;
                }
            }
